...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        mic_embeddings = self.modules.embedding_model(self.modules.mean_var_norm(mic_feats, lens))
        acc_embeddings = self.modules.acc_embedding_model(self.modules.mean_var_norm(accel_feats, lens))

        fused_embeddings = torch.concat((mic_embeddings, acc_embeddings), dim = 2)

        fused_embeddings = self.modules.embedding_fuser(fused_embeddings)
        ...
        outputs = self.modules.classifier(fused_embeddings)
