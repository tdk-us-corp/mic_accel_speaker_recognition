...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        mic_feats = self.compute_DCT(mic_feats, num_MFCC = mic_MFCC)
        accel_feats = self.compute_DCT(accel_feats, num_MFCC = acc_MFCC)
        ...
        # just mic
        # final_feats = self.modules.mean_var_norm(mic_feats, lens)


        # Embeddings + speaker classifier
        mic_embeddings = self.modules.embedding_model(mic_feats)
        accel_embeddings = self.modules.embedding_model(accel_feats)

        all_embeddings = (mic_embeddings + accel_embeddings) / 2

        # print(all_embeddings.shape)

        outputs = self.modules.classifier(all_embeddings)
