...
mic_path = "verif_pairs/test_aug_4+_seed123.txt"
accel_path = "verif_pairs/test_aug_4+_seed123_accel.txt"


base_name = mic_path - ".txt"
...
embedding_dim = 192
p1_embedding_vectors = torch.empty((pairs_len, embedding_dim), dtype = torch.float32, device = "cpu")
p2_embedding_vectors = torch.empty((pairs_len, embedding_dim), dtype = torch.float32, device = "cpu")
# Files have to fully aligned
with open('scores_of_extracted_embs_seed123.txt', 'w') as file:
    for i, (p1, p2) in enumerate(mic_pths[:]):
        ...
        if delta:
            if i == 0:
                print("Delta Enabled")
            # FOR DELTA THE NAMINGS ARE REVERESED DONT PAY ATTENTION ITS JUST TO KEEP THE CONSISTANCY WITH THE REST
            accel_p1_feats = mic_p1_feats[:, 6:, :]
            accel_p2_feats = mic_p2_feats[:, 6:, :]

            mic_p1_feats = mic_p1_feats[:, :6, :] - accel_p1_feats[:, :6, :]
            mic_p2_feats = mic_p2_feats[:, :6, :] - accel_p2_feats[:, :6, :]
        ...
        # p2_emb = compute_embedding(fused_feats_p2)
        ...
        p2_emb = compute_embedding(mic_p2_feats)

        # Just Accel
        # p1_emb = compute_embedding(accel_p1_feats)
        # p2_emb = compute_embedding(accel_p2_feats)

        p1_embedding_vectors[i] = p1_emb.detach().to("cpu")
        p2_embedding_vectors[i] = p2_emb.detach().to("cpu")
        ...
        #     print(f"Error processing pair {p1} and {p2}, {acc_p1}, {acc_p2} \n")
        #     corrupted_counter += 1
        #     mic_labels.pop(i)
        #     acc_labels.pop(i)
        #     continue  # Continue to the next iteration of the loop        

print(f"{corrupted_counter} of corrupted files")
print("All pairs scored. Moving to EER calculation....=")


torch.save(p1_embedding_vectors, 'p1_embeddings_seed123.pt')
torch.save(p2_embedding_vectors, 'p2_embeddings_seed123.pt')
