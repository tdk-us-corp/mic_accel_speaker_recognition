...
dir_path = "/mnt/009-Audio/Internships/AccelAuthentification/chinese_accel_micro_databases/speaker_splits/split1/embeddings/test"
preprocess = True
mic_MFCC = 13
acc_MFCC = 13
audio_norm = True
...
for dir in os.listdir(dir_path):
    ...
    if os.path.isdir(folder_path):
        ...
        for file in os.listdir(folder_path):
            ...
            if file.endswith('acc.pt'):


                print(f'Scoring user {dir}')

                emb_path = os.path.join(folder_path, file)
                user_emb = torch.load(emb_path)


                user_audios_path = folder_path.replace('embeddings/', '/mic/')
                user_acc_audios_path = folder_path.replace('/embeddings/', '/accel/')
                spoofed_user_audios_path = folder_path.replace('/embeddings/', '/spoofed_mic/')
                spoofed_user_acc_audios_path = folder_path.replace('/embeddings/', '/spoofed_acc/')
                

                # Bonafide Audio
                for audio in os.listdir(user_audios_path)[skip_embedding_utterances:skip_embedding_utterances+num_audio_per_speaker]:
                    if audio.endswith('wav'):
                                file_path = os.path.join(user_audios_path, audio)
                                acc_file_path = os.path.join(user_acc_audios_path, 'ACCEL_', audio).replace('ACCEL_/', 'ACCEL_')
                                ...
                                num_samples = mic_audio_p1.shape[1]

                                # Calculate duration in seconds
                                duration_seconds = num_samples / sr

                                if duration_seconds < audio_min_len:
                                     continue
                                ...
                                score = compute_cosine_similarity(user_emb, p1_emb.detach().to('cpu'))
                                all_scores.append(score)
                                all_labels.append(1)
                                all_addresses.append(acc_file_path)
                ...
                # Spoofed audio
                for audio in os.listdir(spoofed_user_audios_path)[skip_embedding_utterances:skip_embedding_utterances+num_audio_per_speaker]:
                    if audio.endswith('wav'):
                                file_path = os.path.join(spoofed_user_audios_path, audio)
                                acc_file_path = os.path.join(spoofed_user_acc_audios_path, 'ACCEL_', audio).replace('ACCEL_/', 'ACCEL_')
...
print(len(all_labels), len(all_scores))


# Calculate EER 
EER, thresh = calculate_eer_and_curves(all_labels, all_scores)

print(len(all_scores), EER, thresh)


print(all_scores[:20])
print(all_scores[20:40])

with open('missclassified_pairs.txt', 'w') as file:
    for j, (lab, score) in enumerate(zip(all_labels, all_scores)):
        file.write(f"{lab} {score}, {all_addresses[j]}\n")
